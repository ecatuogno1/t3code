...
let _cachedTerminalTheme: ITheme | null = null;
let _cachedTerminalThemeIsDark: boolean | null = null;

function terminalThemeFromApp(): ITheme {
  const isDark = document.documentElement.classList.contains("dark");
  if (_cachedTerminalTheme && _cachedTerminalThemeIsDark === isDark) {
    return _cachedTerminalTheme;
  }
        ...
        callback(
          matches.map((match) => ({
            ...
            activate: (event: MouseEvent) => {
              ...
              if (match.kind === "url") {
                const openUrl = match.text.includes("/pull/")
                  ? openBrowserContextUrl(match.text)
                  : api.shell.openExternal(match.text).then(() => true);
                void openUrl.catch((error) => {
                  writeSystemMessage(
                    latestTerminal,
              ...
              void openFileTarget(target)
                .then((opened) => {
                  if (opened) {
                    return;
                  }
                  return openInPreferredEditor(api, target);
                })
                .catch((error) => {
                  writeSystemMessage(
                    latestTerminal,
